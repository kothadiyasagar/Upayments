...
const MapProduct = (props: Data) => {
  ...
  const fevoriteProduvt = (event: any) => {
    ...
    if (appendId) {
      ...
      let c: any = props.elem._id;
      const apendIdData = id.filter(function (elem: any) {
        return elem[0]._id !== c;
      });
      const apendId = productId.filter(function (elem: any) {
        return elem !== c;
      });
      dispatch(like(apendIdData));
      dispatch(likes(apendId));
  ...
  return (
    <>
      <div
        key={props.elem._id}
        className=" ml-7 w-72 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl"
      >
        <img
          onClick={() => {
            navigate(`/product/${props.elem._id}`);
          }}
          src={props.elem.avatar}
          alt="Product"
        ...
        <div className="px-4 py-3 w-72">
          ...
          <p className="text-lg font-bold text-black truncate block capitalize">
            {props.elem.description.split(" ").splice(0, 10).join(" ")}...
          </p>
